...
# ====== 配置 ======
MODEL_PATH = r"D:\python-project\AI_DetectHub\runs\detect\train2\weights\best.pt"  # 改成你的 best.pt
WINDOW_KEYWORD = "BlueStacks"        # 窗口标题包含的关键字（你也可以填 "BlueStacks App Player"）
CONF_THRES = 0.55                    # 置信度阈值
CLS_ID = 0                           # attack 类别ID
DO_CLICK = False                     # True=检测到就自动点击（先观察，确认准了再开）
SHOW_FPS = True
...
def find_window_rect(keyword: str):
    """
...
def main():
    ...
    ok = find_window_rect(WINDOW_KEYWORD)
    if not ok:
        print(f"[ERR] 未找到标题包含 '{WINDOW_KEYWORD}' 的窗口。请先打开 BlueStacks，或修改 WINDOW_KEYWORD。")
        return
    (L, T, R, B), hwnd = ok
    W, H = R - L, B - T
    ...
    while True:
        ...
        frame = np.array(sct.grab(region))[:, :, :3]  # BGRA -> BGR

        # 2) 推理
        results = model.predict(frame, imgsz=640, conf=CONF_THRES, verbose=False)[0]
        ...
        ok2 = find_window_rect(WINDOW_KEYWORD)
        if ok2:
            (L, T, R, B), _ = ok2
            W, H = R - L, B - T
