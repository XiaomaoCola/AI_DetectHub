...
class SceneClassifierTrainer:
    ...
    def __init__(self, config_path: str):
        ...
        self.config_save_path = config['config_save_path']
        ...
        print(f"[INFO] 训练模型: {self.model_name}")
    ...
    def build_model(self) -> None:
        ...
        if self.model_architecture == "resnet18":
            ...
            raise ValueError(f"不支持的模型: {self.model_architecture}")

        self.model = self.model.to(self.device)
        print(f"[INFO] 模型 {self.model_architecture} 已构建完成")
    ...
    def train(self) -> Dict:
        ...
        print(f"[INFO] 开始训练，共 {self.epochs} 个epoch")
        print("=" * 60)
...
def main():
    ...
    trainer = SceneClassifierTrainer(
        config_path="training_configs/BBAutoAttack_scene_classifier.yaml"
    )
